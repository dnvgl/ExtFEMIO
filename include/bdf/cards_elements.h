...
- `PID` : Property identification number of a `PSHELL`, `PCOMP` or
  `PLPLANE` entry. (Integer > 0; Default = `EID`)

- `Gi` : Grid point identification numbers of connection points.
  (Integers > 0, all unique)

- `THETA` : Material property orientation angle in degrees. `THETA` is
  ignored for hyperelastic elements. (Real; Default = 0.0)

- `MCID` : Material coordinate system identification number. The
  x-axis of the material coordinate system is determined by projecting
  the x-axis of the `MCID` coordinate system (defined by the `CORDij`
  entry or zero for the basic coordinate system) onto the surface of
  the element. `MCID` is ignored for hyperelastic elements. (Integer >
  0; if blank, then `THETA` = 0.0 is assumed.)

- `ZOFFS` : Offset from the surface of grid points to the element
  reference plane. `ZOFFS` is ignored for hyperelastic elements.
  (Real)
...
- `Ti` : Membrane thickness of element at grid points G1 through G4.
  If `TFLAG` zero or blank, then `Ti` are actual user specified
  thickness. (Real > 0.0 or blank, not all zero.) If `TFLAG` one, then
  the `Ti` are fraction relative to the `T` value of the `PSHELL`.
  (Real > 0.0 or blank; not all zero. Default = 1.0) `Ti` are ignored
...
- `Ti` : Membrane thickness of element at grid points `G1` through
  `G4`. If `TFLAG` is zero or blank, then `Ti` are actual user
  specified thicknesses. (Real > 0.0 or blank, not all zero.) If
  `TFLAG` is one, then the `Ti` are fractions relative to the `T`
  value of the `PSHELL`. (Real > 0.0 or blank, not all zero. Default =
...
- `X1` , `X2`, `X3` : Components of orientation vector , from `GA`, in
  the displacement coordinate system at `GA` (default), or in the
  basic coordinate system. (Real)

- `G0` : Alternate method to supply the orientation vector using grid
  point `G0`. Direction of is from `GA` to G0. is then transferred to
  End `A`. (Integer > 0; or `GB`)

- `OFFT` : Offset vector interpretation flag. (Character or blank)

- `BIT` : Built-in twist of the cross-sectional axes about the beam
  axis at end `B` relative to end `A`. For beam p-elements only.
  (Real; Default = 0.0)

- `PA`, `PB` : Pin flags for beam ends `A` and `B`, respectively; used
  to remove connections between the grid point and selected
  ...
  with the `PA` and `PB` degrees-of-freedom to be released by the pin
  flags. For example, if `PA` = 4, the `PBEAM` entry must have a
  nonzero value for `J`, the torsional stiffness. (Up to five of the
  unique Integers 1 through 6 with no embedded blanks.) Pin flags are
  not allowed for beam p-elements.

- `W1A`, `W2A`, `W3A`, `W1B`, `W2B`, `W3B` : Components of offset
  vectors from the grid points to the end points of the axis of the
  shear center. (Real; Default = 0.0)

- `SA` , `SB` : Scalar or grid point identification numbers for the
  ends `A` and `B`, respectively. The degrees-of-freedom at these
  points are the warping variables . `SA` and `SB` cannot be specified
...
- `X1`, `X2`, `X3` : Components of orientation vector v , from `GA`,
  in the displacement coordinate system at `GA` (default), or in the
  basic coordinate system. See Remark 8. (Real)

- `G0` : Alternate method to supply the orientation vector v using
  grid point `G0`. The direction of v is from `GA` to `G0`. v is then
  translated to End `A`. (Integer > 0; `G0` ≠ `GA` or `GB`)

- `OFFT` : Offset vector interpretation flag. (character or blank) See
  Remark 8.

- `PA`, `PB`: Pin flags for bar ends A and B, respectively. Used to
  remove connections between the grid point and selected degrees-of-
  ...
  PBAR entry must have a value for J, the torsional stiffness. (Up to
  5 of the unique Integers 1 through 6 anywhere in the field with no
  embedded blanks; Integer > 0.)

- `W1A`, `W2A`, `W3A`, `W1B`, `W2B`, `W3B` : Components of offset
  vectors w_a and w_b , respectively (see Figure 8-8) in displacement
  coordinate systems (or in element system depending upon the content
  of the OFFT field), at points GA and GB, respectively. See Remark 7.
...
- `PID` : Property identification number of a `PROD` entry. (Integer >
  0; Default = `EID`)
