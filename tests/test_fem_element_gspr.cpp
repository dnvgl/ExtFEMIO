/**
   \file test_fem_element_gspr.cpp
   \author Berthold Höllmann <berthold.hoellmann@dnvgl.com>
   \copyright Copyright © 2016 by DNV GL SE
   \brief Tests for reading and writing FEM GSPR element information.
...
TEST_CASE("FEM GSPR element definitions.", "[fem_element_gspr]") {
   ...
   long const ELTYAD = 2;
   std::vector<long> const NODIN ({ 100 });
   ...
   SECTION("check members") {
      ...
      CHECK(probe.processors == std::set<el_processor> ({
               ADVANCE, Preframe, Prefem, Poseidon, general, Sestra, Framework }));
