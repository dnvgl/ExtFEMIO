    ...
    namespace bdf{
      namespace cards{
        ...
        ::std::ostream& operator<<(::std::ostream &os, const cbar &card) {
          ...
          entries.push_back(format<long>(card._EID, card.EID));
          entries.push_back(format<long>(card._PID, card.PID));
          entries.push_back(format<long>(card._GA, card.GA));
          entries.push_back(format<long>(card._GB, card.GB));
          if (card.choose_dir_code == card.has_DCODE) {
            entries.push_back(format<long>(card._G0, card.G0));
            ...
            entries.push_back(format<double>(card._X1, card.X1));
            entries.push_back(format<double>(card._X2, card.X2));
            entries.push_back(format<double>(card._X3, card.X3));
          }

          if (card.OFFT || card.PA || card.PB || card.W1A || card.W2A ||
              card.W3A || card.W1B || card.W2B || card.W3B)
            entries.push_back(format<::std::string>(card._OFFT, card.OFFT));
          else goto cont;

          if (card.PA || card.PB || card.W1A || card.W2A || card.W3A ||
              card.W1B || card.W2B || card.W3B)
            entries.push_back(format<::std::deque<int>>(card._PA, card.PA));
          else goto cont;
          if (card.PB || card.W1A || card.W2A || card.W3A || card.W1B ||
              card.W2B || card.W3B)
            entries.push_back(format<::std::deque<int>>(card._PB, card.PB));
          else goto cont;
          ...
          if (card.W3B)
            entries.push_back(format<double>(card._W3B, card.W3B));
